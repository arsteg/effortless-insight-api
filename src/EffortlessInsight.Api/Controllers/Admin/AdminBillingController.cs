...
public class AdminBillingController : AdminControllerBase
{
    ...
    public async Task<IActionResult> DownloadInvoicePdf(Guid invoiceId)
    {
        ...
        try
        {
            // Use the shared InvoiceService for proper PDF generation (uses QuestPDF)
            var pdfBytes = await _invoiceService.GetInvoicePdfAsync(invoiceId, invoice.OrganizationId);
